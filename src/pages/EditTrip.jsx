...
export default function EditTrip() {
  ...
  const { user } = useAuth();
  const [form, setForm] = useState({ name: '', description: '', cover_photo: '', start_date: '', end_date: '' });
  ...
  if (fetching) return <div className="p-8 text-center"><div className="w-8 h-8 border-2 border-blue-600 border-t-transparent rounded-full animate-spin mx-auto" /></div>;

  return (
    <div className="p-6 lg:p-8 max-w-2xl mx-auto">
      <button onClick={() => navigate(`/trips/${id}`)} className="flex items-center gap-2 text-slate-500 hover:text-slate-700 text-sm font-medium mb-6 transition-colors">
        <ArrowLeft className="w-4 h-4" /> Back to Trip
      </button>
      <div className="mb-8">
        <h1 className="text-2xl font-bold text-slate-900">Edit Trip</h1>
        <p className="text-slate-500 text-sm mt-1">Update your trip details.</p>
      </div>
      ...
      <div className="relative h-48 rounded-2xl overflow-hidden mb-6 bg-gradient-to-br from-blue-400 to-emerald-500">
        ...
        <div className="absolute inset-0 bg-black/20 flex items-center justify-center">
          <div className="text-white text-center"><Camera className="w-8 h-8 mx-auto mb-1 opacity-80" /><p className="text-sm opacity-80">Choose a cover photo below</p></div>
        </div>
      </div>

      <div className="mb-6">
        <label className="text-sm font-medium text-slate-700 mb-2 block">Cover Photo</label>
        ...
        <input name="cover_photo" value={form.cover_photo} onChange={onChange} placeholder="Or paste a custom image URL..." className="mt-2 w-full px-3.5 py-2.5 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 bg-slate-50" />
      </div>

      <form onSubmit={handleSubmit} className="space-y-5">
        {error && <div className="bg-red-50 border border-red-200 text-red-700 rounded-xl px-4 py-3 text-sm">{error}</div>}
        <div>
          <label className="text-sm font-medium text-slate-700 mb-1.5 block">Trip Name *</label>
        ...
        <div>
          ...
          <textarea name="description" value={form.description} onChange={onChange} rows={3} placeholder="Describe your trip..."
            className="w-full px-4 py-3 border border-slate-200 rounded-xl text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500 bg-slate-50 text-sm resize-none" />
        </div>
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            ...
            <input name="end_date" type="date" value={form.end_date} onChange={onChange} min={form.start_date}
              className="w-full px-4 py-3 border border-slate-200 rounded-xl text-slate-900 focus:outline-none focus:ring-2 focus:ring-blue-500 bg-slate-50 text-sm" />
          </div>
        </div>
        <div className="flex gap-3 pt-2">
          <button type="button" onClick={() => navigate(`/trips/${id}`)} className="flex-1 px-5 py-3 border border-slate-200 text-slate-600 rounded-xl text-sm font-medium hover:bg-slate-50 transition-colors">Cancel</button>
          <button type="submit" disabled={loading}
            className="flex-1 flex items-center justify-center gap-2 bg-blue-600 hover:bg-blue-700 disabled:opacity-60 text-white px-5 py-3 rounded-xl text-sm font-medium transition-colors">
